...
// - pkg/resilience has a Client struct with WriteCheckpointSafe, WriteLifecycleEventSafe
// - CheckLastLifecycle() returns (event string, timestamp time.Time, err error)
// - When db == nil, safe wrappers must not panic
// Adapt to your actual pkg/resilience types and function signatures.
...
func TestResilience_NilDB_WriteCheckpointDoesNotPanic(t *testing.T) {
	assert.NotPanics(t, func() {
		// Simulate what your WriteCheckpointSafe should do when db == nil:
		var db interface{} = nil // stand-in for your *pgxpool.Pool
		if db == nil {
			return // safe wrapper returns early — no panic
		}
		// Real write would happen here
