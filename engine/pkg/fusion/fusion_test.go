...
// We now test the internal concepts used by fusion.go natively.

func GetBucketKeyTest(sensorID string, timestamp time.Time, windowDuration time.Duration) bucketKey {
	bucket := timestamp.Truncate(windowDuration)
	return bucketKey{sensorID: sensorID, bucket: bucket.UnixNano()}
}

func MergeFieldsTest(existing, incoming map[string]interface{}) map[string]interface{} {
...
func TestFusion_BucketKey_SameWindowSameBucket(t *testing.T) {
	...
	key1 := GetBucketKeyTest("machine_01", ts1, window)
	key2 := GetBucketKeyTest("machine_01", ts2, window)
...
func TestFusion_BucketKey_DifferentSensorDifferentKey(t *testing.T) {
	...
	key1 := GetBucketKeyTest("machine_01", base, window)
	key2 := GetBucketKeyTest("machine_02", base, window)
...
func TestFusion_MergeFields_TwoSeparatePayloads(t *testing.T) {
	...
	merged := MergeFieldsTest(existing, incoming)
...
func TestFusion_LateArrival_DetectedCorrectly(t *testing.T) {
	...
	assert.True(t, isLate, "Message whose bucket has already been flushed must be treated as late arrival")
