...
// EvaluateSDT wraps the actual EvaluateFieldSDT purely for testing ease.
func EvaluateSDT(state schema.Field, newValue float64, newTime time.Time) (bool, float64, time.Time, schema.Field) {
	shouldEmit, emitValue, emitTime, _ := EvaluateFieldSDT(&state, newValue, newTime)
	return shouldEmit, emitValue, emitTime, state
}
...
func TestSDT_Bootstrap_AlwaysEmitsFirstPoint(t *testing.T) {
	state := schema.Field{DoorWidth: 2.0}
...
func TestSDT_SuddenSpike_EmitsPreviousPoint(t *testing.T) {
	...
	shouldEmit, emitValue, emitTime, _ := EvaluateSDT(state, 100.0, baseTime().Add(2*time.Second))
...
func TestSDT_SlowDrift_BreaksCorridorEventually(t *testing.T) {
	state := schema.Field{DoorWidth: 0.5} // tight door for sensitivity
...
state := schema.Field{DoorWidth: tt.doorWidth}
			emitCount := 0
